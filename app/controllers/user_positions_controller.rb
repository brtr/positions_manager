class UserPositionsController < ApplicationController
  ...
  def update
    ...
    @record.user_positions_notes_histories.create(user_id: current_user.id, notes: record_params[:notes], images: record_params[:images]) if record_params[:notes].present?
