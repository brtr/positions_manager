class GenerateClosingHistoriesSnapshotsJob < ApplicationJob
  ...
  def perform(date: Date.today)
    ...
    generate_snapshot_summary(snapshot_info, date)
  end

  def generate_snapshot(snapshot_info)
    ClosingHistoriesSnapshotRecord.transaction do
      histories = AddingPositionsHistory.closing_data.available
      histories.each do |h|
        snap_shot = snapshot_info.closing_histories_snapshot_records.where(origin_symbol: h.origin_symbol, trade_type: h.trade_type, event_date: snapshot_info.event_date, source: h.source).first_or_create
  ...
  def generate_snapshot_summary(snapshot_info, date)
    total_summary = date == Date.today ? AddingPositionsHistory.closing_data.total_summary : snapshot_info.closing_histories_snapshot_records.total_summary(date: date)
